...
Three immutable assets govern quality cases: `zj-research-rubric-set/v1` defines weighted criteria and Judge thresholds, `zj-research-human-annotation-set/v1` binds every case to evidence verdicts and acceptable recommendation fingerprints or abstention, and `zj-research-judge-calibration-set/v1` pairs blind Judge results with human scores for fixed reports. The runtime admits quality results only when all quality cases resolve across the assets and calibration passes its sample count, score-error, recommendation-agreement, and omitted-risk limits.
