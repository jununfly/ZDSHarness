"""Generate architecture_panorama.html from architecture_panorama.md.

数据源是同目录的 architecture_panorama.md；本脚本把 md 内联进 HTML，
用 marked + mermaid（cdnjs）在浏览器端渲染。改 md 后重跑本脚本同步 HTML。

用法:
  python gen_panorama_html.py
"""
...
MD_FILE = HERE / "architecture_panorama.md"
OUT_FILE = HERE / "architecture_panorama.html"
...
<footer>数据源：architecture_panorama.md（勿手改本 HTML）· 生成于 __TIMESTAMP__</footer>
