<!-- ROADMAP_SECTION_START -->
## ZJ Roadmap

> 数据文件: `learning_roadmap.json` | 最后更新: 2026-08-15 11:26:16

[~][X+] 1. ZHarness 学习计划
├── [~][Y+] 1-1. 跑通开发环境
│   ├── [ ][Y+] 1-1-1. 安装依赖与核心检查
│   ├── [ ][Y+] 1-1-2. 运行 headless 任务
│   └── [ ][Y+] 1-1-3. 运行 Web UI demo
├── [ ][Y+] 1-2. 掌握架构总览
│   ├── [ ][Y+] 1-2-1. 读 docs/architecture.md
│   ├── [ ][Y+] 1-2-2. 读 README 与仓库布局
│   └── [ ][Y+] 1-2-3. 理解一切皆插件哲学
├── [ ][Y+] 1-3. 建立 Cordis 心智模型
│   ├── [ ][Y+] 1-3-1. 完成 cordis-tutorial 7 课
│   ├── [ ][Y+] 1-3-2. 掌握 effects 与事件模型
│   └── [ ][Y+] 1-3-3. 理解服务与配置机制
├── [ ][Y+] 1-4. 按包精读核心代码
│   ├── [ ][Y+] 1-4-1. 精读 packages/core 产品 API
│   ├── [ ][Y+] 1-4-2. 精读一个能力包 (fs/skill/llm)
│   └── [ ][Y+] 1-4-3. 用测试反向理解行为契约
└── [ ][Y+] 1-5. 动手扩展与验证
    ├── [ ][Y+] 1-5-1. 按 cookbook 加一个 tool
    ├── [ ][Y+] 1-5-2. 理解 provider/Consumer 扩展点
    └── [ ][Y+] 1-5-3. 对比 upstream 验证 fork 状态
<!-- ROADMAP_SECTION_END -->
